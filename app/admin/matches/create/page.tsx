// app/admin/matches/create/page.tsx
'use client';

import { useState, useEffect } from 'react';
import { useRouter } from 'next/navigation';
import { Tv, Sparkles, Plus, AlertTriangle, ArrowLeft, Check, X, ShieldAlert, Laptop, RefreshCw } from 'lucide-react';
...
export default function CreateMatchPage() {
  ...
  const [streams, setStreams] = useState<any[]>([
    { serverName: 'Server 1 HD', embedCode: '', isPrimary: true, status: 'ACTIVE' }
  ]);
  ...
  const handleUpdateStream = (idx: number, field: string, value: any) => {
    ...
    if (field === 'isPrimary') {
      // Set all other streams to false, and this one to true
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || 'Gagal menyimpan pertandingan.');
      }

      router.push('/admin');
    } catch (err: any) {
      alert(err.message || 'Terjadi kesalahan saat membuat pertandingan.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] text-zinc-500 space-y-4">
        ...
        <p className="text-xs uppercase tracking-widest">Memuat formulir...</p>
      </div>
    );
  }

  const noTeamsOrLeagues = teams.length === 0 || leagues.length === 0;
    ...
    <div className="max-w-4xl mx-auto space-y-8 select-none">
      ...
      <header className="flex items-center gap-4 border-b border-white/5 pb-6">
        ...
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-1">Konfigurasi detail laga, server streaming, dan metadata SEO.</p>
        </div>
      </header>

      {noTeamsOrLeagues && (
        <div className="p-6 rounded-2xl border border-amber-500/25 bg-amber-500/10 text-amber-300 flex items-start gap-4">
          <AlertTriangle className="w-6 h-6 shrink-0 mt-0.5" />
          <div className="space-y-1">
            <h3 className="text-xs font-black uppercase tracking-wider">PENTING: Data Master Kosong</h3>
            <p className="text-xs text-amber-300/80 leading-relaxed">
              Anda harus menambahkan setidaknya 2 tim dan 1 liga terlebih dahulu sebelum dapat membuat pertandingan.
            </p>
            <div className="flex items-center gap-3 pt-2">
              <Link href="/admin/teams" className="text-xs font-black text-white hover:underline uppercase">Kelola Teams</Link>
              <span className="text-amber-550">•</span>
              <Link href="/admin/leagues" className="text-xs font-black text-white hover:underline uppercase">Kelola Leagues</Link>
            </div>
          </div>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <footer className="flex gap-4">
          ...
          <button
            type="submit"
            disabled={submitting || noTeamsOrLeagues}
          ...
          >
            {submitting ? 'Menyimpan...' : 'Simpan Pertandingan'}
          </button>
        </footer>
      </form>
    </div>
