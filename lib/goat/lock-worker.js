// Worker thread: runs lock.wasm in a happy-dom sandbox to decrypt embed.st blob → m3u8 URL
// Must be plain .js (not TypeScript) — Node.js worker_threads requires a real file path.
...
const EMBED_ORIGIN = process.env.EMBED_ORIGIN ?? 'https://embed.st';

function pageUrl(slot) {
  return `${EMBED_ORIGIN}/embed/${slot.path}`;
}
...
function mountDom(slot) {
  ...
  globalThis.URL = class extends NativeUrl {
    constructor(input, base) {
      if (input === '/fetch') input = `${EMBED_ORIGIN}/fetch`;
      super(input, base ?? `${EMBED_ORIGIN}/`);
    }
  };
...
function patchImports(imports, NativeResponse, goat, body, onM3u8) {
  ...
  // Bypass instanceof checks in WASM
  ...
  // Intercept the WASM internal fetch call
...
async function crack(slot, goat, bodyHex) {
  ...
  const api = await mod.default({
    module_or_path: `${EMBED_ORIGIN}/js/wasm/lock.wasm`,
    fetch: fetchFn,
  });
  await api.init_wasm?.();
  ...
  if (!m3u8) throw new Error('lock.wasm did not yield m3u8 URL');
  return m3u8;
}
